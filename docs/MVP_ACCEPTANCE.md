...
| 模型管理 | 本地/Hugging Face 单文件导入、SHA-256 去重、结构门禁、资料编辑、最近使用/自检记录、旧清单兼容、列表、解析和回收由测试覆盖 |
...
| 游戏稳定性 | 处理耗时/欠载/流错误连续采样驱动观察档和无重载保声旁路；前台全屏/无边框检测会主动进入稳定观察并暂停新后台重任务；长稳脚本将稳态与切档窗口欠载分开判定 |
| 原生质量门 | MSVC 工作区 35 个测试通过；Clippy `-D warnings` 通过 |
