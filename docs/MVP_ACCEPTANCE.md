...
| RVC 真实自检 | 用户提供 Generator + 已校验 ContentVec/RMVPE：DirectML 加载 2173 ms、预热 758 ms、稳态 160 ms 块推理 49.3 ms，并产生非静音 48 kHz 输出 |
...
| 原生质量门 | MSVC 工作区 27 个测试通过；Clippy `-D warnings` 通过 |
...
- 训练环境/工作台已接入但尚未实际完成一次训练；TensorRT RTX 可按需安装但本机仍为 `NotPresent`；RVC v1、非 F0 检查点仍不支持转换。
