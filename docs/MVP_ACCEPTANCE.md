...
| 游戏稳定性 | 处理耗时/欠载/流错误连续采样驱动观察档和无重载保声旁路；前台全屏/无边框检测会主动进入稳定观察并暂停新后台重任务；跨设备 WASAPI 时钟漂移已加入有界低水位补偿，修复后 10 分钟三档循环 P99 56.293 ms 且错误计数为 0 |
| 原生质量门 | MSVC 工作区 66 个测试通过（FoxVoice 41 项、补丁版 `vc-app` 25 项）；Clippy `-D warnings` 通过 |
