...
impl RealtimeConfig {
    ...
    fn pipeline_config<'a>(
        ...
        RvcPipelineConfig {
            ...
            feature_index: self.feature_index.as_deref(),
...
/// fixed-rate bridge will eventually empty its output ring when the render
/// clock is even slightly faster. Below one output chunk, retain a tiny number
/// of samples per callback and stretch the consumed block back to the device
/// callback length. The correction is bounded to about 0.5%, is only active at
/// low water, and preserves a one-sample reserve so ordinary clock drift does
/// not turn into an audible zero-filled gap.
fn drift_corrected_consumption(available: usize, requested: usize, low_water: usize) -> usize {
    if requested == 0 || available == 0 {
        return 0;
    }
    if available >= low_water || requested == 1 {
        return available.min(requested);
    }
    if available == 1 {
        return 0;
    }
    let retained = requested.div_ceil(200).max(1);
    available
        .saturating_sub(1)
        .min(requested.saturating_sub(retained))
}

/// Linearly stretch the initialized prefix across the entire callback buffer.
/// Iterating backwards keeps unread prefix samples intact and performs no
/// allocation in the real-time audio callback.
fn stretch_prefix_in_place(samples: &mut [f32], source_len: usize) {
    if source_len == 0 || samples.is_empty() {
        return;
    }
    if source_len == 1 {
    ...
    if source_len >= samples.len() {
        return;
    }
    let source_span = source_len - 1;
    let output_span = samples.len() - 1;
    for destination in (0..samples.len()).rev() {
        let numerator = destination * source_span;
        let left = numerator / output_span;
        let right = (left + 1).min(source_span);
        let fraction = (numerator % output_span) as f32 / output_span as f32;
        samples[destination] = samples[left] + (samples[right] - samples[left]) * fraction;
    }
}
...
fn build_streams(
    ...
    let output_stream = audio.build_output_stream(move |out| {
        ...
        // `cached_slots()` only reflects the consumer's last local refresh. If
        // we decide not to pop, it can stay at zero while the producer fills
        // the ring, causing false underruns and eventually producer drops.
        // `slots()` synchronizes the producer cursor before the decision.
        let available = output_consumer.slots();
        let consume = drift_corrected_consumption(available, out.len(), low_water);
        if consume == 0 {
            out.fill(last_output_sample);
        } else {
            let (_, remainder) = output_consumer.pop_partial_slice(&mut out[..consume]);
            debug_assert!(remainder.is_empty());
            stretch_prefix_in_place(out, consume);
            last_output_sample = out.last().copied().unwrap_or(last_output_sample);
        }
        if available == 0 {
            output_telemetry
                .output_underruns
...
mod tests {
    ...
    #[test]
    fn clock_drift_correction_is_bounded_and_keeps_a_reserve() {
        assert_eq!(drift_corrected_consumption(0, 480, 7_680), 0);
        assert_eq!(drift_corrected_consumption(1, 480, 7_680), 0);
        assert_eq!(drift_corrected_consumption(480, 480, 7_680), 477);
        assert_eq!(drift_corrected_consumption(20_000, 480, 7_680), 480);
    }

    #[test]
    fn clock_drift_stretch_preserves_endpoints_without_allocating() {
        let mut samples = vec![0.0, 1.0, 2.0, 0.0, 0.0];
        stretch_prefix_in_place(&mut samples, 3);
