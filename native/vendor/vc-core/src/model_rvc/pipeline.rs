...
pub struct RvcPipeline {
    ...
    retrieved_feature_tensor: FeatureTensor,
...
pub struct RvcPipelineConfig<'a> {
    ...
    pub f0_model: &'a Path,
    pub feature_index: Option<&'a Path>,
    pub index_rate: f32,
    /// Retained retrieval strength on unvoiced consonants (standard RVC: 0.33).
    pub protect: f32,
...
impl RvcPipeline {
    ...
    pub fn load(config: RvcPipelineConfig<'_>) -> Result<Self> {
        if !(0.0..=1.0).contains(&config.index_rate) {
            bail!("index_rate must be between 0 and 1");
        }
        if !(0.0..=0.5).contains(&config.protect) {
            bail!("protect must be between 0 and 0.5");
        }
        ...
        Ok(Self {
            ...
            retrieved_feature_tensor: FeatureTensor::default(),
            feature_index: config.feature_index.map(|path| FeatureIndex::load(path, expected_feat_channels)).transpose()?,
...
impl VoiceModel for RvcPipeline {
    ...
    fn process(
        ...
        if let Some(index) = self.feature_index.as_mut() {
            index.retrieve_tensor(
                &self.feature_tensor.data,
                &self.feature_tensor.shape,
                &mut self.retrieved_feature_tensor.data,
            )?;
            self.retrieved_feature_tensor.shape.clone_from(&self.feature_tensor.shape);
        }
        ...
        if silence_front_frames > 0 && silence_front_frames < feature_len_before_trim {
            if silence_front_frames.is_multiple_of(2) {
                ...
                self.retrieved_feature_tensor.trim_front_frames(silence_front_frames / 2)?;
                self.retrieved_feature_tensor.repeat_frames(2)?;
            } else {
                self.feature_tensor.repeat_frames(2)?;
                self.feature_tensor
                    .trim_front_frames(silence_front_frames)?;
                self.retrieved_feature_tensor.repeat_frames(2)?;
                self.retrieved_feature_tensor.trim_front_frames(silence_front_frames)?;
            }
        } else {
            self.feature_tensor.repeat_frames(2)?;
            if self.feature_index.is_some() { self.retrieved_feature_tensor.repeat_frames(2)?; }
        }
        ...
        if self.feature_index.is_some() && self.index_rate > 0.0 {
            let channels = usize::try_from(self.feature_tensor.shape[2]).context("invalid feature channels")?;
            for (frame_index, (original, retrieved)) in self.feature_tensor.data.chunks_exact_mut(channels)
                .zip(self.retrieved_feature_tensor.data.chunks_exact(channels)).enumerate() {
                let voice_weight = if pitchf.get(frame_index).copied().unwrap_or(0.0) > 0.0 { 1.0 } else { self.protect };
                let rate = self.index_rate * voice_weight;
                for (value, indexed) in original.iter_mut().zip(retrieved) { *value = *value * (1.0 - rate) + *indexed * rate; }
            }
        }
        debug!(
